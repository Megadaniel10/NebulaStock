...
class MarketAsset {
  constructor(type, ticker, name, basePrice, volatility, sector, mcap, desc, extra = {}, isPro = false) {
    ...
    this.extra = {
      ceo: extra.ceo || (type === 'crypto' ? 'Decentralizzato' : 'Sconosciuto'),
      founded: extra.founded || (type === 'crypto' ? '2009' : '2015'),
      employees: extra.employees || (type === 'crypto' ? 'N/A' : '100+'),
  ...
  exportState() {
    return { currentPrice: this.currentPrice, history: this.history };
  }
...
export default function App() {
  ...
  const [newAsset, setNewAsset] = useState({ type: 'stocks', ticker: '', name: '', price: 10, vol: 0.02, sector: 'Tech', mcap: '$1M', desc: '', ceo: '', founded: '', employees: '', dividend: '0.00%' });
  ...
  useEffect(() => {
    // Inizializzazione degli asset base con info aggiuntive
    const defaultAssets = {
      ...
      'MINE': new MarketAsset('stocks', 'MINE', 'Minemarket', 12.0, 0.05, 'Estrazione', '$350M', 'Attrezzature da miniera.', { ceo: 'Luca M.', founded: '2005', employees: '890', dividend: '4.10%' }),
      ...
      'GCRD': new MarketAsset('stocks', 'GCRD', 'Garda Credits', 30.0, 0.02, 'Finanza', '$600M', 'Recupero crediti.', { ceo: 'Antonio S.', founded: '2012', employees: '200', dividend: '0.00%' }),
      'MRCH': new MarketAsset('stocks', 'MRCH', 'Marchetti', 200.0, 0.01, 'Lusso', '$4.8B', 'Holding moda.', { ceo: 'Silvia M.', founded: '1985', employees: '3,400', dividend: '2.80%' }),
      'CGOV': new MarketAsset('stocks', 'CGOV', 'Governo', 5.0, 0.08, 'Rischi', '$10B', 'Asset volatile politico.', { ceo: 'Parlamento', founded: '1946', employees: 'N/A', dividend: '0.00%' }),
      'ESTI': new MarketAsset('crypto', 'ESTI', 'EsticazziCoin', 0.05, 0.1, 'Meme', '$40M', 'Disinteresse globale.', { ceo: 'Nessuno', founded: '2024', employees: '0', dividend: '0.00%' }),
      'BTC': new MarketAsset('crypto', 'BTC', 'Bitcoin', 65000.0, 0.005, 'Crypto', '$1.2T', 'Oro digitale.', { ceo: 'Satoshi Nakamoto', founded: '2009', employees: '0', dividend: '0.00%' }),
      'ETH': new MarketAsset('crypto', 'ETH', 'Ethereum', 3500.0, 0.008, 'Crypto', '$400B', 'Infrastruttura Web3.', { ceo: 'Vitalik Buterin', founded: '2015', employees: '0', dividend: '0.00%' }, true)
    };
    ...
    if (saved) { 
      try { 
        ...
        // Ripristina gli asset customizzati
        if (loadedState.customAssets) {
          loadedState.customAssets.forEach(ca => {
            assetsRef.current[ca.ticker] = new MarketAsset(ca.type, ca.ticker, ca.name, ca.price, ca.volatility, ca.sector, ca.mcap, ca.desc, { ceo: ca.ceo, founded: ca.founded, employees: ca.employees, dividend: ca.dividend });
          });
        }

        // Ripristina la cronologia esatta e i prezzi per evitare che il mercato si resetti
        if (loadedState.assetsData) {
          Object.keys(loadedState.assetsData).forEach(ticker => {
            if (assetsRef.current[ticker]) {
              assetsRef.current[ticker].importState(loadedState.assetsData[ticker]);
            }
          });
        }
      } catch (e) {} 
  ...
  // Sync state & market data to localStorage su ogni tick del mercato
  useEffect(() => {
    if (!isLoaded) return;
    const assetsData = {};
    Object.keys(assetsRef.current).forEach(ticker => {
      assetsData[ticker] = assetsRef.current[ticker].exportState();
    });
    const stateToSave = { ...state, assetsData };
    localStorage.setItem('nebulaState', JSON.stringify(stateToSave));
  }, [state, isLoaded, tick]);

  // Aggiornamento Mercato (2 secondi)
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      setTick(t => t + 1);
    }, 2000); // Modificato a 2000ms come richiesto
  ...
  const createCustomAsset = () => {
    ...
    const custom = new MarketAsset(newAsset.type, ticker, newAsset.name, parseFloat(newAsset.price), parseFloat(newAsset.vol), newAsset.sector, newAsset.mcap, newAsset.desc, { ceo: newAsset.ceo, founded: newAsset.founded, employees: newAsset.employees, dividend: newAsset.dividend });
    ...
    setNewAsset({ type: 'stocks', ticker: '', name: '', price: 10, vol: 0.02, sector: 'Tech', mcap: '$1M', desc: '', ceo: '', founded: '', employees: '', dividend: '0.00%' });
    showToast(`${newAsset.type === 'stocks' ? 'Azienda' : 'Crypto'} ${ticker} aggiunta con successo.`, "success");
  };
      ...
      <div className="flex flex-1 overflow-hidden">
        <nav className="w-16 md:w-64 bg-nebula-900 border-r border-nebula-border flex flex-col justify-between shrink-0 z-10">
          ...
          <div className="p-4 flex justify-center md:justify-start">
            <button onClick={() => { if (prompt("Password:") === "Diegoègay") { setState(prev => ({...prev, ui: {...prev.ui, activeTab: 'admin'}})); showToast("Dev Mode", "success"); } }} className={`transition-colors ${state.ui.activeTab === 'admin' ? 'text-rose-500' : 'text-nebula-700 hover:text-slate-500'}`} title="Dev Mode"><Lock className="w-4 h-4" /></button>
          </div>
        </nav>
        ...
        <main className="flex-1 bg-nebula-950 overflow-hidden relative">
          ...
          <div className={`h-full flex-col md:flex-row w-full ${state.ui.activeTab === 'markets' ? 'flex' : 'hidden'}`}>
            <div className="w-full md:w-80 border-r border-nebula-border bg-nebula-900 flex flex-col shrink-0">
              ...
              <div className="flex-1 overflow-y-auto custom-scroll p-2 space-y-1">
                {Object.values(assetsRef.current).filter(a => a.type === state.ui.activeMarketType).map(asset => (
                  <div key={asset.ticker} onClick={() => setState(p => ({...p, ui: {...p.ui, activeAsset: asset.ticker}}))} className={`cursor-pointer p-3 rounded-lg border flex justify-between items-center transition-colors ${asset.ticker === state.ui.activeAsset ? 'bg-nebula-800 border-nebula-700' : 'border-transparent hover:bg-nebula-800/50'}`}>
                    <div>
            ...
            <div className="flex-1 flex flex-col h-full overflow-y-auto custom-scroll">
              <div className="p-4 md:p-6 border-b border-nebula-border bg-nebula-900/50 flex flex-col md:flex-row justify-between md:items-start gap-4">
                <div className="flex flex-col flex-1">
                  ...
                  <div className="flex items-center space-x-4 mt-2 text-xs">
                    ...
                    <span className="text-slate-400 flex items-center"><Coins className="w-3 h-3 mr-1 text-slate-500"/>Cap: <span className="font-mono ml-1">{activeAssetObj.mcap}</span></span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 text-xs border-t border-nebula-border/50 pt-4">
                    <div className="flex flex-col"><span className="text-slate-500 mb-1 flex items-center"><UserCog className="w-3 h-3 mr-1"/> CEO</span><span className="font-semibold text-slate-300 truncate">{activeAssetObj.extra.ceo}</span></div>
                    <div className="flex flex-col"><span className="text-slate-500 mb-1 flex items-center"><Building className="w-3 h-3 mr-1"/> Fondazione</span><span className="font-semibold text-slate-300">{activeAssetObj.extra.founded}</span></div>
                    <div className="flex flex-col"><span className="text-slate-500 mb-1 flex items-center"><Users className="w-3 h-3 mr-1"/> Dipendenti</span><span className="font-semibold text-slate-300">{activeAssetObj.extra.employees}</span></div>
                    <div className="flex flex-col"><span className="text-slate-500 mb-1 flex items-center"><Percent className="w-3 h-3 mr-1"/> Dividendo</span><span className="font-semibold text-slate-300">{activeAssetObj.extra.dividend}</span></div>
                  </div>
                </div>
              ...
              <div className="p-4 md:p-6 border-b border-nebula-border bg-[#05070f] relative">
                <div className="absolute top-6 right-6 z-10 flex space-x-2 bg-nebula-900 p-1 rounded-lg border border-nebula-border items-center">
                ...
                <div className="relative w-full h-[350px] border border-nebula-border rounded-xl bg-nebula-950 overflow-hidden" onWheel={handleChartZoom}><canvas ref={chartCanvasRef} className="absolute inset-0 w-full h-full cursor-crosshair"></canvas></div>
              </div>
              ...
              <div className="p-4 md:p-6 grid grid-cols-1 lg:grid-cols-2 gap-6 bg-nebula-950 flex-1">
                <div className="bg-nebula-900/40 p-5 rounded-xl border border-nebula-border relative overflow-hidden">
                  {(activeAssetObj.type === 'stocks' && (state.gameTime.hours < 9 || state.gameTime.hours >= 18)) && (
                    <div className="absolute inset-0 bg-nebula-950/80 z-10 flex flex-col items-center justify-center backdrop-blur-sm"><Store className="w-10 h-10 text-rose-500 mb-2" /><span className="text-rose-400 font-bold">Mercato Azionario Chiuso</span></div>
                  )}
                  ...
                  <div className="mb-4"><input type="number" value={tradeQty} min="1" onChange={(e) => setTradeQty(e.target.value)} className="w-full bg-nebula-900 border border-nebula-border rounded-xl px-4 py-3 font-mono text-white text-lg font-bold outline-none focus:border-cyan-500" /></div>
                ...
                <div className="bg-nebula-900/40 p-5 rounded-xl border border-nebula-border">
                  ...
                  <div className="space-y-4 font-mono text-sm">
                    <div className="flex justify-between border-b border-nebula-border pb-2"><span className="text-slate-400">Asset Posseduti</span><span className="text-white font-bold">{actShares}</span></div>
              ...
              <div ref={chatScrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
                {(state.chatHistory[state.ui.activeChatRoom] || []).map((m, idx) => (
                  <div key={idx} className="flex flex-col space-y-1 mb-2">
                    ...
                    <div className="text-sm p-2 rounded-lg bg-nebula-800/50 whitespace-pre-wrap" style={{color: m.textCol}}>{m.text}</div>
                  </div>
                ))}
              </div>
              <div className="p-4 bg-nebula-900 border-t border-nebula-border">
                <form onSubmit={sendMessage} className="relative">
                  <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Scrivi un messaggio..." className="w-full bg-nebula-950 border border-nebula-border rounded-xl pl-4 pr-12 py-3 text-white focus:outline-none focus:border-cyan-500" autoComplete="off" />
          ...
          <div className={`h-full flex-col p-6 overflow-y-auto custom-scroll w-full ${state.ui.activeTab === 'settings' ? 'flex' : 'hidden'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl">
              <div className="bg-nebula-900/40 p-6 border border-nebula-border rounded-xl">
                ...
                <label className="block text-xs text-slate-400 mb-1">ID Discord</label><input type="text" disabled value={state.user.id} className="w-full bg-nebula-950/50 border border-nebula-border rounded-lg px-3 py-2 text-slate-400 font-mono mb-4" />
                <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-nebula-900/40 p-6 border border-amber-500/30 md:col-span-2 relative rounded-xl">
                ...
                <div className="flex space-x-2">
                  <input type="text" id="pro-key-input" placeholder="XXXX-XXXX" className="flex-1 bg-nebula-950 border border-amber-500/50 rounded-lg px-4 py-2 text-white font-mono uppercase focus:outline-none" />
                  <button onClick={() => {
                  ...
                  }} className="px-6 py-2 bg-gradient-to-r from-amber-600 to-orange-500 text-white font-bold rounded-lg hover:opacity-90">Riscatta</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className={`h-full flex-col p-6 overflow-y-auto custom-scroll w-full ${state.ui.activeTab === 'admin' ? 'flex' : 'hidden'}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-nebula-900/40 p-6 border border-rose-900/50 rounded-xl">
                <h3 className="text-lg font-bold text-white mb-4 border-b border-rose-900/50 pb-2">Forza Saldo Locale</h3>
                <div className="flex items-center space-x-4">
                  <input type="number" id="dev-db-cash" defaultValue={state.portfolio.cash} className="bg-nebula-950 border border-nebula-border rounded px-3 py-2 text-white font-mono flex-1" />
                  <button onClick={() => { const c = parseFloat(document.getElementById('dev-db-cash').value); if(!isNaN(c)) { setState(p => ({...p, portfolio: {...p.portfolio, cash: c}})); syncBalanceWithBackend(c); showToast("Aggiornato", "success"); } }} className="py-2 px-6 bg-slate-700 hover:bg-slate-600 text-white font-bold rounded">Aggiorna</button>
                </div>
              </div>

              <div className="bg-nebula-900/40 p-6 border border-rose-900/50 rounded-xl lg:col-span-2">
                <h3 className="text-lg font-bold text-white mb-4 border-b border-rose-900/50 pb-2">Creazione Asset (Aziende / Crypto)</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  <div><label className="text-xs text-slate-400 block mb-1">Mercato</label><select value={newAsset.type} onChange={e => setNewAsset({...newAsset, type: e.target.value})} className="w-full bg-nebula-950 border border-nebula-border rounded px-3 py-2 text-white text-sm"><option value="stocks">Azioni</option><option value="crypto">Crypto</option></select></div>
                  <div><label className="text-xs text-slate-400 block mb-1">Ticker (es. GOOG)</label><input type="text" maxLength="4" value={newAsset.ticker} onChange={e => setNewAsset({...newAsset, ticker: e.target.value})} className="w-full bg-nebula-950 border border-nebula-border rounded px-3 py-2 text-white text-sm uppercase" /></div>
                  ...
                  <div><label className="text-xs text-slate-400 block mb-1">Prezzo Iniziale ($)</label><input type="number" step="0.1" value={newAsset.price} onChange={e => setNewAsset({...newAsset, price: e.target.value})} className="w-full bg-nebula-950 border border-nebula-border rounded px-3 py-2 text-white text-sm" /></div>
                  ...
                  <div><label className="text-xs text-slate-400 block mb-1">Anno Fondazione</label><input type="text" value={newAsset.founded} onChange={e => setNewAsset({...newAsset, founded: e.target.value})} className="w-full bg-nebula-950 border border-nebula-border rounded px-3 py-2 text-white text-sm" /></div>
                  ...
                  <div className="md:col-span-4"><label className="text-xs text-slate-400 block mb-1">Descrizione</label><input type="text" value={newAsset.desc} onChange={e => setNewAsset({...newAsset, desc: e.target.value})} className="w-full bg-nebula-950 border border-nebula-border rounded px-3 py-2 text-white text-sm" /></div>
                </div>
                <button onClick={createCustomAsset} className="w-full py-3 bg-rose-600 hover:bg-rose-500 text-white font-bold rounded-lg transition-colors">Aggiungi al Mercato</button>
              </div>
      ...
      {discordModal.open && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-nebula-900/90 border border-cyan-500/50 rounded-2xl max-w-sm w-full p-6 relative">
            ...
            <button onClick={executeDiscordTransaction} className="w-full py-3 bg-[#5865F2] hover:bg-[#4752C4] text-white rounded-lg font-bold">Conferma su Discord</button>
          </div>
        </div>
      )}
